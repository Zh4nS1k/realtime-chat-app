...
import { MessageBubble, TypingIndicator, ChatListItem } from '@/components/ios';
...
const translations = {
  ru: {
    ...
    chooseChatSubtitle:
      'DM для личной переписки или группа минимум с тремя участниками.',
...
export default function Home() {
  ...
  const iosColors = getIOSColors(theme);
  const colors = iosColors.avatar;
  ...
  const sendTextMessage = async (payload: {
    ...
    if (!payload.content && !payload.imageUrl && !payload.fileUrl) return;
    setSending(true);
    try {
      if (socket) {
  ...
  if (loading) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[var(--ios-bg)]">
        <div className="ios-glass rounded-3xl px-8 py-6 text-[var(--ios-text-primary)] shadow-lg">
          <div className="flex items-center gap-3">
            <span className="h-5 w-5 animate-spin rounded-full border-2 border-[var(--ios-blue)]/30 border-t-[var(--ios-blue)]" />
            <span className="text-[15px] font-medium">{copy.loading}</span>
          </div>
        </div>
      </main>
    );
  ...
  return (
    <main className="min-h-screen bg-[var(--ios-bg)] text-[var(--ios-text-primary)] ios-animate-fade-in">
      <div className="flex h-screen overflow-hidden">
        ...
        <div className="relative flex min-h-[75vh] gap-6">
          ...
          >
            <div className="flex items-center justify-between rounded-xl border border-[var(--border)] bg-[var(--muted)]/35 px-3 py-2">
              <div>
                <p className="text-xs uppercase text-[var(--foreground)]/60">
                  {copy.status}
                </p>
            ...
            <div className="rounded-xl border border-[var(--border)] bg-[var(--muted)]/25 p-3 shadow-sm">
              <div className="flex items-center gap-2">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--card)] text-lg">
                  🔍
                </div>
                <input
                  value={searchTerm}
                ...
                <div className="flex gap-2 rounded-full bg-[var(--muted)]/40 p-1 text-xs font-semibold text-[var(--foreground)]">
                  <button
                    ...
                    className={`rounded-full px-3 py-1 ${
                      activeFilter === 'all' ? 'bg-[var(--card)] shadow-sm' : ''
                    }`}
                  >
                    All
                  </button>
                  <button
                    ...
                    className={`rounded-full px-3 py-1 ${
                      activeFilter === 'dm' ? 'bg-[var(--card)] shadow-sm' : ''
                    }`}
                  >
                    DM
                  </button>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs text-[var(--foreground)]/60">
                            {conversation.lastMessage
                              ...
                              : dayjs(
                                  conversation.updatedAt || conversation._id
                                ).fromNow()}
                          </span>
                        </div>
                ...
                <div className="flex items-center justify-between border-b border-[var(--border)] px-6 py-4">
                  ...
                  <div className="flex items-center gap-2 text-xs text-[var(--foreground)]/70">
                    ...
                    <div className="ml-3 flex items-center gap-2 text-base">
                      <button className="rounded-full bg-[var(--muted)]/60 px-2 py-1 hover:bg-[var(--muted)]">
                        🔍
                      </button>
                      <button className="rounded-full bg-[var(--muted)]/60 px-2 py-1 hover:bg-[var(--muted)]">
                        📞
                        ...
                        >
                          ...
                          {!message.imageUrl && message.fileUrl && (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="mt-2 flex items-center gap-3 rounded-xl border border-white/20 bg-black/10 px-3 py-2 text-xs text-inherit hover:bg-black/20"
                            >
                          ...
                          <div
                            className={`mt-2 flex items-center gap-2 text-[10px] ${statusClass}`}
                          >
                            <span>
                              {dayjs(message.createdAt).format('HH:mm')}
                            </span>
                ...
                <div className="sticky bottom-0 border-t border-[var(--border)] bg-[var(--card)] px-6 py-4">
                  <div className="flex flex-wrap items-center gap-3">
                    <UploadButton<ChatFileRouter>
                      ...
                      onUploadBegin={() => setUploadProgress(5)}
                    ...
                    <button
                      onClick={() =>
                        sendTextMessage({
                          ...
                          imageUrl: attachment?.type?.startsWith('image/')
                            ? attachment?.url
                            : undefined,
                          fileUrl: !attachment?.type?.startsWith('image/')
                            ? attachment?.url
                            : undefined,
                  ...
                  {attachment && (
                    <div className="mt-3 flex items-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--card)] px-3 py-2">
                      <div className="relative h-16 w-16 overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--muted)]/40">
                        {previewImage ? (
                          <Image
                            src={previewImage}
                            alt="preview"
                      ...
                      <div className="flex-1">
                        ...
                        {uploadProgress >= 100 && (
                          <div className="mt-1 text-[10px] uppercase text-emerald-500">
                            Uploaded
                          </div>
                        )}
                      </div>
        ...
        {error && (
          <div className="fixed bottom-6 right-6 z-50 ios-animate-slide-up">
            <div className="ios-glass rounded-2xl border border-[var(--ios-red)]/30 bg-[var(--ios-red)]/90 px-5 py-3.5 shadow-xl backdrop-blur-xl">
              <div className="flex items-center gap-3">
                <span className="text-lg">⚠️</span>
                <p className="flex-1 text-[15px] font-semibold text-white">
                  {error}
                </p>
                <button
                  onClick={() => setError(null)}
                  className="ios-button flex h-7 w-7 items-center justify-center rounded-full bg-white/20 text-white hover:bg-white/30"
                  aria-label="Close"
                >
                  ×
                </button>
...
function Lightbox({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md ios-animate-fade-in"
      onClick={onClose}
    >
      <button
        onClick={onClose}
        className="ios-button absolute right-6 top-6 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-lg font-semibold text-white backdrop-blur-sm hover:bg-white/20"
        aria-label="Close"
      >
        ✕
      </button>
      <div
        className="relative max-h-[90vh] max-w-[90vw] ios-animate-scale-in"
        ...
        <div className="space-y-2 p-4">
          {conversation.participants.map((p) => (
            ...
            >
              ...
              <div className="text-[10px] uppercase text-[var(--foreground)]/60">
                Member
              </div>
            </div>
          ))}
